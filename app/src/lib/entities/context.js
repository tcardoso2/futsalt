import ALIGN from '../../lib/canvas/canvas-1.2.min'
import Ball from './ball'
import Match from '../entities/match'
import SceneManager from '../entities/scene'
import Subscriber from '../entities/subscriber'
//Will remove in future
//import $ from '../jquery.min-3.4.1'
...
class Context extends Subscriber{
    ...
    */                          
    ...
    addToScene(object) {
        this.getScene().addToScene(object)
    }

    drawScene() {
        this.getScene().draw(this.canvas)
    }
    /*
     ____        _ _ 
    |  _ \      | | |
    ...
    */
    ...
    addPlayer(player) {
        ...
        player.subscribe((content) => {
            ...
            let lastContent = ''//TODO, replace by react $('.footer-comments').html()
    ...
    movePlayer(playerName) {
        this.players[playerName].moveTowards(this.ball, (objective) => {
            ...
            //TODO, replace by react $('.footer-comments').html(error)
        })
    }
    ...
    challenge(ball, challenger) {
        ...
        this.pauseMatch()
        this.getScene().changeTo("VS")
        let self = this
        challenger.matchUp(ball.getOwner(), (done) => {                setTimeout(() => {
            self.getScene().changeTo("main")
                self.resumeMatch()
            }, 3000)
        })
    }
